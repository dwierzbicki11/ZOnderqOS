...
1. D2.1 — controller discovery and binding
...
2. D2.2 — BAR/MMIO validation and controller capability probing
...
The existing QEMU launcher already knows how to attach `ich9-ahci` and `nvme` devices, so D2 should reuse those device models for runtime validation rather than invent a synthetic hardware environment. The first implementation checkpoint is D2.1 controller classification/binding and a dedicated CI/runtime gate; D2.2 must not start until D2.1 is green.
