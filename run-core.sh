...
apply_cpu_profile() {
    ...
    if [[ "$arch" == "x64" ]]; then
        case "$profile" in
            1) HW_CPU_MODEL="Conroe"; HW_CPU_LABEL="Intel Core 2 Duo E6600 (Conroe)"; HW_CORES=2; HW_THREADS=1 ;;
            2) HW_CPU_MODEL="Nehalem"; HW_CPU_LABEL="Intel Core i7-920 (Nehalem)"; HW_CORES=4; HW_THREADS=2 ;;
            3) HW_CPU_MODEL="SandyBridge"; HW_CPU_LABEL="Intel Core i7-2600 (Sandy Bridge)"; HW_CORES=4; HW_THREADS=2 ;;
            4) HW_CPU_MODEL="IvyBridge"; HW_CPU_LABEL="Intel Core i7-3770 (Ivy Bridge)"; HW_CORES=4; HW_THREADS=2 ;;
            5) HW_CPU_MODEL="Haswell"; HW_CPU_LABEL="Intel Core i7-4770 (Haswell)"; HW_CORES=4; HW_THREADS=2 ;;
            6) HW_CPU_MODEL="Broadwell"; HW_CPU_LABEL="Intel Core i7-5775C (Broadwell)"; HW_CORES=4; HW_THREADS=2 ;;
            7) HW_CPU_MODEL="Skylake-Client"; HW_CPU_LABEL="Intel Core i7-6700K (Skylake)"; HW_CORES=4; HW_THREADS=2 ;;
            8) HW_CPU_MODEL="Cascadelake-Server"; HW_CPU_LABEL="Intel Xeon Platinum 8280 (Cascade Lake)"; HW_CORES=28; HW_THREADS=2 ;;
            9) HW_CPU_MODEL="EPYC-Rome"; HW_CPU_LABEL="AMD EPYC 7302 (Rome)"; HW_CORES=16; HW_THREADS=2 ;;
            10) HW_CPU_MODEL="EPYC-Milan"; HW_CPU_LABEL="AMD EPYC 7443 (Milan)"; HW_CORES=24; HW_THREADS=2 ;;
            *) fail "Nieprawidlowy profil x64: $profile" ;;
        esac
    else
        case "$profile" in
            1) HW_CPU_MODEL="cortex-a35"; HW_CPU_LABEL="NXP i.MX 8QuadXPlus / Cortex-A35"; HW_CORES=4; HW_THREADS=1 ;;
            2) HW_CPU_MODEL="cortex-a53"; HW_CPU_LABEL="Raspberry Pi 3 BCM2837 / Cortex-A53"; HW_CORES=4; HW_THREADS=1 ;;
            3) HW_CPU_MODEL="cortex-a55"; HW_CPU_LABEL="Rockchip RK3568 / Cortex-A55"; HW_CORES=4; HW_THREADS=1 ;;
            4) HW_CPU_MODEL="cortex-a57"; HW_CPU_LABEL="NVIDIA Tegra X1 A57 cluster / Cortex-A57"; HW_CORES=4; HW_THREADS=1 ;;
            5) HW_CPU_MODEL="cortex-a72"; HW_CPU_LABEL="Raspberry Pi 4 BCM2711 / Cortex-A72"; HW_CORES=4; HW_THREADS=1 ;;
            6) HW_CPU_MODEL="cortex-a76"; HW_CPU_LABEL="Raspberry Pi 5 BCM2712 / Cortex-A76"; HW_CORES=4; HW_THREADS=1 ;;
            7) HW_CPU_MODEL="cortex-a710"; HW_CPU_LABEL="Snapdragon 8 Gen 1 A710 cluster / Cortex-A710"; HW_CORES=3; HW_THREADS=1 ;;
            8) HW_CPU_MODEL="neoverse-n1"; HW_CPU_LABEL="AWS Graviton2 / Neoverse-N1"; HW_CORES=64; HW_THREADS=1 ;;
            9) HW_CPU_MODEL="neoverse-v1"; HW_CPU_LABEL="AWS Graviton3 / Neoverse-V1"; HW_CORES=64; HW_THREADS=1 ;;
            10) HW_CPU_MODEL="a64fx"; HW_CPU_LABEL="Fujitsu A64FX"; HW_CORES=48; HW_THREADS=1 ;;
...
select_cpu_profile() {
    ...
    local choice
    if [[ "$arch" == "x64" ]]; then default_profile=7; else default_profile=5; fi
    print_cpu_menu "$arch"
...
prompt_ram() {
    ...
    local default_ram
    local entered
    if [[ "$arch" == "x64" ]]; then default_ram="2G"; else default_ram="512M"; fi
    echo
...
set_advanced_defaults() {
    local arch="$1"
    if [[ "$arch" == "x64" ]]; then apply_cpu_profile x64 7; HW_RAM="2G"; else apply_cpu_profile arm64 5; HW_RAM="512M"; fi
}

parse_hardware_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --sockets) [[ $# -ge 2 ]] || fail "--sockets wymaga wartosci."; HW_SOCKETS="$2"; shift 2 ;;
            --cores) [[ $# -ge 2 ]] || fail "--cores wymaga wartosci."; HW_CORES="$2"; shift 2 ;;
            --threads) [[ $# -ge 2 ]] || fail "--threads wymaga wartosci."; HW_THREADS="$2"; shift 2 ;;
            --ram) [[ $# -ge 2 ]] || fail "--ram wymaga wartosci."; HW_RAM="$2"; shift 2 ;;
            --cpu) [[ $# -ge 2 ]] || fail "--cpu wymaga wartosci."; HW_CPU_MODEL="$2"; HW_CPU_LABEL="Custom: $2"; shift 2 ;;
            -h|--help) usage; exit 0 ;;
...
validate_hardware() {
    ...
    if [[ "$HW_RAM" =~ ^[1-9][0-9]*$ ]]; then HW_RAM="${HW_RAM}M"; fi
    HW_RAM="${HW_RAM^^}"; HW_RAM="${HW_RAM%B}"
    [[ "$HW_RAM" =~ ^[1-9][0-9]*[KMGTPE]$ ]] || fail "Nieprawidlowy RAM '$HW_RAM'. Przyklady: 512M, 2G, 8G."
    HW_VCPUS=$((HW_SOCKETS * HW_CORES * HW_THREADS))
    (( HW_VCPUS >= 1 && HW_VCPUS <= 128 )) || fail "Laczna liczba watkow/vCPU musi byc 1..128 (jest: $HW_VCPUS)."
    [[ -n "$HW_CPU_MODEL" ]] || fail "Model CPU nie moze byc pusty."
}

verify_cpu_model() {
    local arch="$1"; local emulator
    if [[ "$arch" == "x64" ]]; then emulator="qemu-system-x86_64"; else emulator="qemu-system-aarch64"; fi
    require_command "$emulator"
    "$emulator" -cpu help 2>/dev/null | grep -Fq "$HW_CPU_MODEL" || fail "QEMU nie zna modelu CPU '$HW_CPU_MODEL' dla $arch. Sprawdz: $emulator -cpu help"
}
...
print_hardware_summary() {
    ...
    echo "[QEMU] Topologia: ${HW_SOCKETS} socket x ${HW_CORES} core x ${HW_THREADS} thread/core = ${HW_VCPUS} logicznych CPU"
...
run_x64() {
    ...
    local -a qemu_args=(-L "$qemu_data" -M q35 -cpu "$HW_CPU_MODEL" -smp "$smp" -m "$HW_RAM" -drive "file=$iso,if=none,id=cosmoscd,format=raw,readonly=on" -device ide-cd,drive=cosmoscd,bootindex=0 -boot d -display gtk,zoom-to-fit=on -full-screen -serial stdio)
    if [[ -f "$ROOT_DIR/zonder_disk.img" || -f "$ROOT_DIR/disk_sata_1G.img" ]]; then qemu_args+=( -device ich9-ahci,id=ahci0 ); fi
    if [[ -f "$ROOT_DIR/zonder_disk.img" ]]; then qemu_args+=( -drive "file=$ROOT_DIR/zonder_disk.img,if=none,id=ahcidisk0,format=raw" -device ide-hd,drive=ahcidisk0,bus=ahci0.0 ); fi
    if [[ -f "$ROOT_DIR/disk_sata_1G.img" ]]; then qemu_args+=( -drive "file=$ROOT_DIR/disk_sata_1G.img,if=none,id=ahcidisk1,format=raw" -device ide-hd,drive=ahcidisk1,bus=ahci0.1 ); fi
    if [[ -f "$ROOT_DIR/disk_nvme_2G.img" ]]; then qemu_args+=( -drive "file=$ROOT_DIR/disk_nvme_2G.img,if=none,id=nvmedisk0,format=raw" -device nvme,drive=nvmedisk0,serial=nvme-1 ); fi
    qemu-system-x86_64 "${qemu_args[@]}"
}

find_arm64_firmware() {
    local -a candidates=("$HOME/.cosmos/tools/qemu/share/qemu/edk2-aarch64-code.fd" "$HOME/.cosmos/tools/share/qemu/edk2-aarch64-code.fd" "/usr/share/qemu-efi-aarch64/QEMU_EFI.fd" "/usr/share/AAVMF/AAVMF_CODE.fd" "/usr/share/edk2/aarch64/QEMU_EFI.fd")
    local candidate
    for candidate in "${candidates[@]}"; do [[ -f "$candidate" ]] && { printf '%s\n' "$candidate"; return 0; }; done
...
run_arm64() {
    ...
    build_iso arm64
    local iso="$BUILD_ISO"
    local firmware; firmware="$(find_arm64_firmware)"
    echo
    ...
    local smp="cpus=${HW_VCPUS},sockets=${HW_SOCKETS},cores=${HW_CORES},threads=${HW_THREADS}"
    local -a qemu_args=(-M virt,gic-version=3,highmem-ecam=off,highmem-mmio=off -cpu "$HW_CPU_MODEL" -smp "$smp" -m "$HW_RAM" -bios "$firmware" -drive "if=none,id=cd,file=$iso,format=raw,readonly=on" -device virtio-scsi-pci -device scsi-cd,drive=cd,bootindex=0 -device virtio-keyboard-device -device virtio-mouse-device -device virtio-gpu-pci,xres=1920,yres=1080 -display gtk,zoom-to-fit=on -serial stdio -no-reboot -no-shutdown)
    if [[ -f "$ROOT_DIR/zonder_disk.img" ]]; then qemu_args+=( -drive "file=$ROOT_DIR/zonder_disk.img,if=none,id=armroot,format=raw" -device nvme,drive=armroot,serial=zonderq-arm-root ); elif [[ -f "$ROOT_DIR/disk_nvme_2G.img" ]]; then qemu_args+=( -drive "file=$ROOT_DIR/disk_nvme_2G.img,if=none,id=armroot,format=raw" -device nvme,drive=armroot,serial=zonderq-arm-root ); else echo "[ARM64] UWAGA: brak persistent disk image; VFS nie bedzie mial partycji root." >&2; fi
...
sync_repo
choice="${1:-}"
if [[ -z "$choice" ]]; then echo "1) x86_64 - full ZonderqOS + QEMU"; echo "2) ARM64  - full ZonderqOS + QEMU virt"; echo; read -r -p "Wybierz [1/2]: " choice; else shift; fi
case "$choice" in
    1|x64|x86|x86_64) arch="x64" ;;
    2|arm64|arm|qemu-arm64) arch="arm64" ;;
    -h|--help|help) usage; exit 0 ;;
    *) fail "Nieprawidlowy wybor: $choice. Uzyj ./run.sh --help" ;;
esac
if [[ $# -eq 0 ]]; then select_cpu_profile "$arch"; prompt_ram "$arch"; else set_advanced_defaults "$arch"; parse_hardware_args "$@"; fi
validate_hardware
echo
echo "Wybrana konfiguracja:"
print_hardware_summary "$arch"
echo
case "$arch" in x64) run_x64 ;; arm64) run_arm64 ;; esac
