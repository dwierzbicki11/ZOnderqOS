...
# stability gate. Stage 16 reuses it, requires the complete Stage-15 stack
# proof, and additionally validates one real scheduler dispatch on every AP.
CHECK_LOG=0
if [[ "${1:-}" == --check-log ]]; then
  CHECK_LOG=1
  shift
...
LOG="${2:-stage16-qemu-serial.log}"
CPUS="${STAGE16_QEMU_CPUS:-${STAGE14_QEMU_CPUS:-8}}"

export STAGE14_QEMU_CPU_MODEL="${STAGE16_QEMU_CPU_MODEL:-${STAGE14_QEMU_CPU_MODEL:-Nehalem}}"
export STAGE14_QEMU_SOCKETS="${STAGE16_QEMU_SOCKETS:-${STAGE14_QEMU_SOCKETS:-1}}"
export STAGE14_QEMU_CPUS="$CPUS"
export STAGE14_QEMU_CORES="${STAGE16_QEMU_CORES:-${STAGE14_QEMU_CORES:-4}}"
export STAGE14_QEMU_THREADS="${STAGE16_QEMU_THREADS:-${STAGE14_QEMU_THREADS:-2}}"
export STAGE14_QEMU_TIMEOUT_SECONDS="${STAGE16_QEMU_TIMEOUT_SECONDS:-${STAGE14_QEMU_TIMEOUT_SECONDS:-90}}"
export STAGE14_QEMU_STABLE_SECONDS="${STAGE16_QEMU_STABLE_SECONDS:-${STAGE14_QEMU_STABLE_SECONDS:-30}}"

fail() {
  echo "[SMT16-QEMU][FAIL] $*" >&2
  exit 1
}

[[ "$CPUS" =~ ^[1-9][0-9]{0,3}$ ]] || fail "CPUS must be an integer in 1..9999 (got: $CPUS)"
(( CPUS <= 256 )) || fail 'Stage 16 supports at most 256 logical CPUs'
command -v python3 >/dev/null || fail 'python3 not found'

check_stage16_serial() {
  python3 - "$1" "$CPUS" <<'PY'
import pathlib
import re
import sys
...
def fail(message):
    print("[SMT16-QEMU][FAIL] " + message, file=sys.stderr)
    raise SystemExit(1)
...
print(
    f"[SMT16-QEMU] Serial proof: {expected_aps} AP idle stack(s) enrolled with "
...
print(
    f"[SMT16-QEMU] Scheduler proof: {expected_aps} AP worker thread(s) ran on "
