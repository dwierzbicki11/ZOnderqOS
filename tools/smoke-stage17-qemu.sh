...
# stability gate. Stage 17 reuses it, requires the Stage-15 stack and Stage-16
...
LOG="${2:-stage17-qemu-serial.log}"
CPUS="${STAGE17_QEMU_CPUS:-${STAGE14_QEMU_CPUS:-8}}"

export STAGE14_QEMU_CPU_MODEL="${STAGE17_QEMU_CPU_MODEL:-${STAGE14_QEMU_CPU_MODEL:-Nehalem}}"
export STAGE14_QEMU_SOCKETS="${STAGE17_QEMU_SOCKETS:-${STAGE14_QEMU_SOCKETS:-1}}"
export STAGE14_QEMU_CPUS="$CPUS"
export STAGE14_QEMU_CORES="${STAGE17_QEMU_CORES:-${STAGE14_QEMU_CORES:-4}}"
export STAGE14_QEMU_THREADS="${STAGE17_QEMU_THREADS:-${STAGE14_QEMU_THREADS:-2}}"
export STAGE14_QEMU_TIMEOUT_SECONDS="${STAGE17_QEMU_TIMEOUT_SECONDS:-${STAGE14_QEMU_TIMEOUT_SECONDS:-90}}"
export STAGE14_QEMU_STABLE_SECONDS="${STAGE17_QEMU_STABLE_SECONDS:-${STAGE14_QEMU_STABLE_SECONDS:-30}}"

fail() {
  echo "[SMT17-QEMU][FAIL] $*" >&2
  exit 1
}

[[ "$CPUS" =~ ^[1-9][0-9]{0,3}$ ]] || fail "CPUS must be an integer in 1..9999 (got: $CPUS)"
(( CPUS <= 256 )) || fail 'Stage 17 supports at most 256 logical CPUs'
command -v python3 >/dev/null || fail 'python3 not found'

check_stage17_serial() {
  python3 - "$1" "$CPUS" <<'PY'
import pathlib
import re
import sys
...
def fail(message):
    print("[SMT17-QEMU][FAIL] " + message, file=sys.stderr)
    raise SystemExit(1)
...
print(
    f"[SMT17-QEMU] Serial proof: {expected_aps} AP idle stack(s) enrolled with "
...
print(
    f"[SMT17-QEMU] Stage-16 regression: {expected_aps} AP worker thread(s) ran on "
...
print(
    f"[SMT17-QEMU] Preemption proof: {expected_aps} live AP worker thread(s) "
    "were preempted by their local LAPIC timer, returned to per-CPU idle, "
    "then resumed from the saved IRQ context and exited cleanly"
)
PY
}
...
check_stage17_serial "$LOG"
echo '[SMT17-QEMU][OK] Stage 14-16 regressions and Stage 17 AP timer preemption survived the stability interval.'
