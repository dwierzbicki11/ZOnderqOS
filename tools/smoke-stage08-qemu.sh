...
LOG="${2:-stage08-qemu-serial.log}"
CPU_MODEL="${STAGE08_QEMU_CPU_MODEL:-Nehalem}"
SOCKETS="${STAGE08_QEMU_SOCKETS:-1}"
CPUS="${STAGE08_QEMU_CPUS:-8}"
CORES="${STAGE08_QEMU_CORES:-4}"
THREADS="${STAGE08_QEMU_THREADS:-2}"
TIMEOUT="${STAGE08_QEMU_TIMEOUT_SECONDS:-90}"
STABLE_SECONDS="${STAGE08_QEMU_STABLE_SECONDS:-30}"

fail() {
  echo "[SMT8-QEMU][FAIL] $*" >&2
  exit 1
}
...
(( CPUS <= 256 )) || fail 'Stage 8 supports at most 256 logical CPUs'
...
def fail(message):
    print("[SMT8-QEMU][FAIL] " + message, file=sys.stderr)
    raise SystemExit(1)
...
print(f"[SMT8-QEMU] Serial proof: {expected} dense CPU(s), {len(ready)} AP(s) READY, {len(ipi)} AP(s) acknowledged initial fixed IPI, {len(run)} AP(s) completed repeated rendezvous, {len(idle)} AP(s) executed parked commands, distinct stacks and GS IDs verified; last-tick={ticks[-1]}")
PY
}
...
: > "$QEMU_STDERR"
echo "[SMT8-QEMU] ${CPU_MODEL}: ${SOCKETS}S/${CORES}C/${THREADS}T = ${CPUS} logical CPU(s); stability=${STABLE_SECONDS}s"
qemu-system-x86_64 \
  -M q35 -accel tcg \
  -cpu "$CPU_MODEL" \
...
while :; do
  ...
  if proof="$(check_serial "$LOG")"; then
    ...
    if (( ready_at < 0 )); then
      ...
      echo "$proof"
      echo "[SMT8-QEMU] Observing another ${STABLE_SECONDS}s for late faults or warnings..."
    fi
    ...
    if (( SECONDS - ready_at >= STABLE_SECONDS )); then
      ...
      echo '[SMT8-QEMU][OK] Parked AP command wake and BSP timer survived the stability interval.'
      echo "[SMT8-QEMU][OK] BSP timer advanced from tick $first_observed_tick to $current_tick."
      if (( CPUS == 1 )); then
        echo '[SMT8-QEMU][OK] Single-CPU regression only: no application-processor worker exists in this topology.'
      else
        echo "[SMT8-QEMU][OK] $((CPUS - 1)) AP(s) executed one native parked command after IPI wake; no AP scheduler is claimed."
      fi
      exit 0
    fi
  else
    ...
    if (( SECONDS - started >= TIMEOUT )); then
      cat "$QEMU_STDERR" "$LOG" >&2
      fail "timeout after ${TIMEOUT}s waiting for complete Stage 8 evidence"
    fi
  fi
  sleep 1
done
