...
LOG="${2:-stage05-qemu-serial.log}"
CPU_MODEL="${STAGE05_QEMU_CPU_MODEL:-Nehalem}"
SOCKETS="${STAGE05_QEMU_SOCKETS:-1}"
CPUS="${STAGE05_QEMU_CPUS:-8}"
CORES="${STAGE05_QEMU_CORES:-4}"
THREADS="${STAGE05_QEMU_THREADS:-2}"
TIMEOUT="${STAGE05_QEMU_TIMEOUT_SECONDS:-90}"
STABLE_SECONDS="${STAGE05_QEMU_STABLE_SECONDS:-30}"

fail() {
  echo "[SMT5-QEMU][FAIL] $*" >&2
  exit 1
}
...
(( CPUS <= 256 )) || fail 'Stage 5 supports at most 256 logical CPUs'
(( STABLE_SECONDS >= 30 )) || fail 'Stage 5 requires at least 30 seconds after AP work proof and first timer tick'
...
def fail(message):
    print("[SMT5-QEMU][FAIL] " + message, file=sys.stderr)
    raise SystemExit(1)
...
worker_re = re.compile(r"\[SMP-TEST\] cpu=(\d+) counter=(\d+)")
for line in lines:
    ...
    if line.startswith("[SMP] AP READY:"):
        ...
        ready[cpu] = lapic
        stacks.add(stack)
    if line.startswith("[SMP-TEST] cpu="):
        match = worker_re.fullmatch(line)
        if not match:
            fail("malformed AP worker result: " + line)
        cpu, counter = int(match[1]), int(match[2])
        if cpu not in range(1, expected) or cpu in workers:
            fail("out-of-range or duplicate AP worker CpuId: " + line)
        if counter != 100000:
            fail("AP worker counter did not reach exactly 100000: " + line)
        workers[cpu] = counter
    ...
    if line.startswith("[SMP] Native work proof complete:") and line != (
        f"[SMP] Native work proof complete: {expected - 1} AP worker(s); APs are now parked with interrupts disabled."
    ):
...
if (any(marker not in lines for marker in required)
        ...
        or set(workers) != set(range(1, expected))
        or 1 not in ticks):
    raise SystemExit(2)

print(f"[SMT5-QEMU] Serial proof: {expected} dense CPU(s), {len(ready)} AP(s) READY, {len(workers)} native AP worker(s) reached counter=100000, distinct stacks and GS IDs verified; last-tick={ticks[-1]}")
PY
}
...
: > "$QEMU_STDERR"
echo "[SMT5-QEMU] ${CPU_MODEL}: ${SOCKETS}S/${CORES}C/${THREADS}T = ${CPUS} logical CPU(s); stability=${STABLE_SECONDS}s"
qemu-system-x86_64 \
  -M q35 -accel tcg \
  -cpu "$CPU_MODEL" \
...
while :; do
  ...
  if proof="$(check_serial "$LOG")"; then
    ...
    if (( ready_at < 0 )); then
      ...
      echo "$proof"
      echo "[SMT5-QEMU] Observing another ${STABLE_SECONDS}s for late faults or warnings..."
    fi
    ...
    if (( SECONDS - ready_at >= STABLE_SECONDS )); then
      ...
      echo '[SMT5-QEMU][OK] Native AP work proof and BSP timer survived the stability interval.'
      echo "[SMT5-QEMU][OK] BSP timer advanced from tick $first_observed_tick to $current_tick."
      if (( CPUS == 1 )); then
        echo '[SMT5-QEMU][OK] Single-CPU regression only: no application-processor worker exists in this topology.'
      else
        echo "[SMT5-QEMU][OK] $((CPUS - 1)) AP(s) executed the bounded native counter task and are parked; no AP scheduler or interrupt support is claimed."
      fi
      exit 0
    fi
  else
    ...
    if (( SECONDS - started >= TIMEOUT )); then
      cat "$QEMU_STDERR" "$LOG" >&2
      fail "timeout after ${TIMEOUT}s waiting for complete Stage 5 evidence"
    fi
  fi
  sleep 1
done
