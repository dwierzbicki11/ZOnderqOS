...
LOG="${2:-stage04-qemu-serial.log}"
CPU_MODEL="${STAGE04_QEMU_CPU_MODEL:-Nehalem}"
SOCKETS="${STAGE04_QEMU_SOCKETS:-1}"
CPUS="${STAGE04_QEMU_CPUS:-8}"
CORES="${STAGE04_QEMU_CORES:-4}"
THREADS="${STAGE04_QEMU_THREADS:-2}"
TIMEOUT="${STAGE04_QEMU_TIMEOUT_SECONDS:-90}"
STABLE_SECONDS="${STAGE04_QEMU_STABLE_SECONDS:-30}"

fail() {
  echo "[SMT4-QEMU][FAIL] $*" >&2
  exit 1
}
...
(( CPUS <= 256 )) || fail 'Stage 4 supports at most 256 logical CPUs'
(( STABLE_SECONDS >= 30 )) || fail 'Stage 4 requires at least 30 seconds after AP readiness and first timer tick'
...
def fail(message):
    print("[SMT4-QEMU][FAIL] " + message, file=sys.stderr)
    raise SystemExit(1)
...
print(f"[SMT4-QEMU] Serial proof: {expected} dense CPU(s), {len(ready)} AP(s) READY, distinct stacks and GS IDs verified; last-tick={ticks[-1]}")
PY
}
...
: > "$QEMU_STDERR"
echo "[SMT4-QEMU] ${CPU_MODEL}: ${SOCKETS}S/${CORES}C/${THREADS}T = ${CPUS} logical CPU(s); stability=${STABLE_SECONDS}s"
qemu-system-x86_64 \
  -M q35 -accel tcg \
  -cpu "$CPU_MODEL" \
...
while :; do
  ...
  if proof="$(check_serial "$LOG")"; then
    ...
    if (( ready_at < 0 )); then
      ...
      echo "$proof"
      echo "[SMT4-QEMU] Observing another ${STABLE_SECONDS}s for late faults or warnings..."
    fi
    ...
    if (( SECONDS - ready_at >= STABLE_SECONDS )); then
      ...
      echo '[SMT4-QEMU][OK] Stage 4 bootstrap and BSP timer survived the stability interval.'
      echo "[SMT4-QEMU][OK] BSP timer advanced from tick $first_observed_tick to $current_tick."
      if (( CPUS == 1 )); then
        echo '[SMT4-QEMU][OK] Single-CPU regression only: no application processors exist in this topology.'
      else
        echo "[SMT4-QEMU][OK] $((CPUS - 1)) AP(s) executed native bootstrap and remain parked; no AP scheduler or interrupt support is claimed."
      fi
      exit 0
    fi
  else
    ...
    if (( SECONDS - started >= TIMEOUT )); then
      cat "$QEMU_STDERR" "$LOG" >&2
      fail "timeout after ${TIMEOUT}s waiting for complete Stage 4 evidence"
    fi
  fi
  sleep 1
done
