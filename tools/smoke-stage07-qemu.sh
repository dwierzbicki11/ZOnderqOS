...
LOG="${2:-stage07-qemu-serial.log}"
CPU_MODEL="${STAGE07_QEMU_CPU_MODEL:-Nehalem}"
SOCKETS="${STAGE07_QEMU_SOCKETS:-1}"
CPUS="${STAGE07_QEMU_CPUS:-8}"
CORES="${STAGE07_QEMU_CORES:-4}"
THREADS="${STAGE07_QEMU_THREADS:-2}"
TIMEOUT="${STAGE07_QEMU_TIMEOUT_SECONDS:-90}"
STABLE_SECONDS="${STAGE07_QEMU_STABLE_SECONDS:-30}"

fail() {
  echo "[SMT7-QEMU][FAIL] $*" >&2
  exit 1
}
...
(( CPUS <= 256 )) || fail 'Stage 7 supports at most 256 logical CPUs'
(( STABLE_SECONDS >= 30 )) || fail 'Stage 7 requires at least 30 seconds after AP work proof and first timer tick'
...
def fail(message):
    print("[SMT7-QEMU][FAIL] " + message, file=sys.stderr)
    raise SystemExit(1)
...
print(f"[SMT7-QEMU] Serial proof: {expected} dense CPU(s), {len(ready)} AP(s) READY, {len(ipi)} AP(s) acknowledged initial fixed IPI, {len(run)} AP(s) completed repeated rendezvous, distinct stacks and GS IDs verified; last-tick={ticks[-1]}")
PY
}
...
: > "$QEMU_STDERR"
echo "[SMT7-QEMU] ${CPU_MODEL}: ${SOCKETS}S/${CORES}C/${THREADS}T = ${CPUS} logical CPU(s); stability=${STABLE_SECONDS}s"
qemu-system-x86_64 \
  -M q35 -accel tcg \
  -cpu "$CPU_MODEL" \
...
while :; do
  ...
  if proof="$(check_serial "$LOG")"; then
    ...
    if (( ready_at < 0 )); then
      ...
      echo "$proof"
      echo "[SMT7-QEMU] Observing another ${STABLE_SECONDS}s for late faults or warnings..."
    fi
    ...
    if (( SECONDS - ready_at >= STABLE_SECONDS )); then
      ...
      echo '[SMT7-QEMU][OK] Repeated fixed IPI rendezvous and BSP timer survived the stability interval.'
      echo "[SMT7-QEMU][OK] BSP timer advanced from tick $first_observed_tick to $current_tick."
      if (( CPUS == 1 )); then
        echo '[SMT7-QEMU][OK] Single-CPU regression only: no application-processor worker exists in this topology.'
      else
        echo "[SMT7-QEMU][OK] $((CPUS - 1)) AP(s) acknowledged four total fixed IPIs across Stage 6 and Stage 7; no AP scheduler is claimed."
      fi
      exit 0
    fi
  else
    ...
    if (( SECONDS - started >= TIMEOUT )); then
      cat "$QEMU_STDERR" "$LOG" >&2
      fail "timeout after ${TIMEOUT}s waiting for complete Stage 7 evidence"
    fi
  fi
  sleep 1
done
