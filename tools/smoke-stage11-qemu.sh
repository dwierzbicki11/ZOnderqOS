...
LOG="${2:-stage11-qemu-serial.log}"
CPU_MODEL="${STAGE11_QEMU_CPU_MODEL:-Nehalem}"
SOCKETS="${STAGE11_QEMU_SOCKETS:-1}"
CPUS="${STAGE11_QEMU_CPUS:-8}"
CORES="${STAGE11_QEMU_CORES:-4}"
THREADS="${STAGE11_QEMU_THREADS:-2}"
TIMEOUT="${STAGE11_QEMU_TIMEOUT_SECONDS:-90}"
STABLE_SECONDS="${STAGE11_QEMU_STABLE_SECONDS:-30}"

fail() {
  echo "[SMT11-QEMU][FAIL] $*" >&2
  exit 1
}
...
(( CPUS <= 256 )) || fail 'Stage 11 supports at most 256 logical CPUs'
(( STABLE_SECONDS >= 30 )) || fail 'Stage 11 requires at least 30 seconds after real OrionGC stop-the-world proof and first timer tick'
...
def fail(message):
    print("[SMT11-QEMU][FAIL] " + message, file=sys.stderr)
    raise SystemExit(1)
...
print(f"[SMT11-QEMU] Serial proof: {expected} dense CPU(s), {len(ready)} AP(s) READY, Stage-10 rendezvous preserved, 3 real OrionGC collections passed, runtime-STW stops={len(gc_stops)} resumes={len(gc_resumes)}; last-tick={ticks[-1]}")
PY
}
...
: > "$QEMU_STDERR"
echo "[SMT11-QEMU] ${CPU_MODEL}: ${SOCKETS}S/${CORES}C/${THREADS}T = ${CPUS} logical CPU(s); stability=${STABLE_SECONDS}s"
qemu-system-x86_64 \
  -M q35 -accel tcg \
  -cpu "$CPU_MODEL" \
...
while :; do
  ...
  if proof="$(check_serial "$LOG")"; then
    ...
    if (( ready_at < 0 )); then
      ...
      echo "$proof"
      echo "[SMT11-QEMU] Observing another ${STABLE_SECONDS}s for late faults or warnings..."
    fi
    ...
    if (( SECONDS - ready_at >= STABLE_SECONDS )); then
      ...
      echo '[SMT11-QEMU][OK] Real OrionGC stop-the-world integration and BSP timer survived the stability interval.'
      echo "[SMT11-QEMU][OK] BSP timer advanced from tick $first_observed_tick to $current_tick."
      if (( CPUS == 1 )); then
        echo '[SMT11-QEMU][OK] Single-CPU regression only: no application-processor worker exists in this topology.'
      else
        echo "[SMT11-QEMU][OK] $((CPUS - 1)) AP(s) executed a parked command and delivered >=3 private periodic LAPIC timer ticks; no AP scheduler is claimed."
      fi
      exit 0
    fi
  else
    ...
    if (( SECONDS - started >= TIMEOUT )); then
      cat "$QEMU_STDERR" "$LOG" >&2
      fail "timeout after ${TIMEOUT}s waiting for complete Stage 11 real-GC evidence"
    fi
  fi
  sleep 1
done
