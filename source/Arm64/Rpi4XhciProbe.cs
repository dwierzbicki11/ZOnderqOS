...
namespace ZonderqOS
{
    ...
    ///
    /// BCM2711 PCI config space is not ordinary ECAM. PFTF selects a BDF through
    /// PCIE_REG_BASE + 0x9000 and exposes that function's 4 KiB config page at
    /// PCIE_REG_BASE + 0x8000. We use that same quirk here.
    ///
    /// Only PCI Memory Space decoding is enabled. IRQs, DMA, controller reset,
    /// xHCI rings, USB enumeration and keyboard input remain disabled.
    /// </summary>
    internal static unsafe class Rpi4XhciProbe
    {
        ...
        public static void Run()
        {
            ...
            Console.WriteLine("  PCI requested in csproj: true");
            Console.WriteLine("  Cosmos generic PCI init: OFF (IRQ cascade)");
            Console.WriteLine("  BCM2711 manual PCI probe: ON");
            Console.WriteLine();
            ...
            if (secondaryBus == 0)
            {
                Console.WriteLine("  secondary bus is zero; stopping.");
                return;
            }

            // The onboard VL805 is the single endpoint immediately downstream
            // from the BCM2711 root port: device 0, function 0.
            ...
            {
                Console.WriteLine("  " + secondaryBus + ":0.0 vendor: " + Hex(vendorId));
            ...
            {
                Console.WriteLine("  endpoint is not xHCI; stopping.");
                return;
            }
            ...
            // PFTF XHC0 _DSM says RegisterAccessType=1: use 32-bit register accesses.
        ...
        private static void FlushMappings()
        {
            Rpi4PageTableNative.DsbIsb();
